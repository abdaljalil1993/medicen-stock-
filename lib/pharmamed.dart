...
class Pharmamed extends StatefulWidget {
  String name;
  Pharmamed(this.name);
  @override
  _PharmamedState createState() => _PharmamedState();
}

class _PharmamedState extends State<Pharmamed> {
  var item = [];
  Future alloffer() async {
    var res = await http.get(Uri.parse(
        "https://shazahinnawi.000webhostapp.com/apis/pharmamed.php?sname=" +
            widget.name));
      ...
      body: Padding(
        ...
        child: ListView.builder(
          ...
          itemBuilder: (context, i) {
            return listitem(
                item[i]['stockname'], item[i]['medname'], item[i]['qte']);
          },
        ),
      ),
    );
